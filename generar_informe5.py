...
# ==============================
# AJUSTAR IMAGEN CON MARGEN Y MEJORAR CALIDAD
# ==============================
def ajustar_imagen_con_margen(ruta, marco, color_fondo=(255,255,255)):
    """
    Ajusta la imagen para que se vea completa dentro del marco,
    manteniendo proporción y usando un tamaño seguro para evitar MemoryError.
    Además aplica mejoras de nitidez y contraste para que no se vea borrosa.
    """
    import os
    ...
    # Escalar dimensiones para PIL sin exceder memoria
    MAX_PIX = 3000
    ...
    # Redimensionar imagen con LANCZOS
    img_resized = img.resize((new_w, new_h), Image.LANCZOS)

    # Nitidez extra
    sharp = ImageEnhance.Sharpness(img_resized)
    img_resized = sharp.enhance(2.0)  # un poco más

    # Contraste
    contrast = ImageEnhance.Contrast(img_resized)
    img_resized = contrast.enhance(1.25)

    # Opcional: ajustar brillo para que no se vea opaca
    brightness = ImageEnhance.Brightness(img_resized)
    img_resized = brightness.enhance(1.05)

    # Fondo del tamaño exacto de la imagen escalada
    fondo = Image.new("RGB", (new_w, new_h), color_fondo)
    left = (new_w - img_resized.width) // 2
    top = (new_h - img_resized.height) // 2
    fondo.paste(img_resized, (left, top))

    # Guardar imagen temporal con buena calidad
    ruta_temp = os.path.join(os.path.dirname(ruta), "_temp_img.png")
    #fondo.save(ruta_temp, dpi=(300,300), quality=95)
    fondo.save(ruta_temp, format="PNG", dpi=(300,300))

    # Devolver coordenadas y tamaño reales del marco para insertar en PPT
    left_ppt = marco.left
    top_ppt = marco.top
    width_ppt = marco.width
    height_ppt = marco.height

    return ruta_temp, left_ppt, top_ppt, width_ppt, height_ppt
...
def mejorar_imagen(ruta):
    ...
    base_width = 2000  # menor que antes para evitar memoria
